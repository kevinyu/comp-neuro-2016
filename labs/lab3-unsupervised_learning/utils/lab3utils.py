...
def initialize(dataset,numOutputs=1,learningRate=0.1):
    """
    Initialize a training run for any of the Hebb-stle rules in lab3_1
    ...
    learningRate         : float, initial learningRate divided by batch size
    
    also, draws the dataset with plottedWeightVectors on top
    """
    
    numInputs,numDatapoints = dataset.shape
    learningRate = learningRate/numDatapoints
    ...
    return figure,plottedWeightVectors,weights,learningRate

def animFrame(_,learnFunk,dataset,plottedWeightVectors,weights,learningRate):
    """
    ...
    Parameters
    ----------
    _                    : int, unused frame index, provided by FuncAnimation
    dataset              : numpy array, either D1 or D2
    plottedWeightVectors : matplotlib line object, will update data during animation
    weights              : numpy array, weight matrix of linear transformation of input data
    learningRate         : float, initial learningRate divided by batch size
    ...
    but draws into figure provided to FuncAnimation
    """
...
def initializeWTA(dataset,goodWeights=False,numOutputs=1,learningRate=0.1):
    """
...
def faceInitialize(dataset,numOutputs=1,learningRate=5*10**-8):
    """
    ...
    learningRate : float, factor to multiply gradient by
    
    Returns
    -------
    dataset      : numpy array, mean-subtracted yalefaces
    weights      : numpy array, dataset.shape[0] by numOutputs
    learningRate : float, provided learningRate divided by batchsize
    ...
    return dataset,weights,learningRate
